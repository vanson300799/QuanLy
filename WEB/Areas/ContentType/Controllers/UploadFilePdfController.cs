...
{
    public partial class UploadFilePdfController : BaseController
    {
        ...
        public ActionResult WebContent_Read([DataSourceRequest] DataSourceRequest request, int id)
        {
            ...
            var contents = db.WebContents.Where(x => x.WebModuleID == id).Select(x => new { x.ID, x.Title, x.Link, x.ModifiedBy, x.ModifiedDate, x.PublishDate });
        ...
        public ActionResult Add(WebContent model, HttpPostedFileBase file, HttpPostedFileBase image)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    model.Link = file.FileSave(0, "/uploads/pdf/" + (now.Month.ToString("00") + now.Year));
