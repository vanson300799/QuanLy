...
$(document).ready(function () {
    try{
        var cleft = $('#left').text().killWhiteSpace().length;
        if (cleft == 0) {
            hideNav();
        }
    } catch (e) { hideNav(); }
...
function adminsitemap(ak) {
    $(document).ready(function () {
        if ($('.breadcrumbs').html() == "") {
            $.getJSON('/admin/base/_adminsitemap?ak=' + ak).done(function (data) {
                var jdata = $.parseJSON(data.Json);
                var len = jdata.length;
                if (len > 0) {
                    var html = "";
                    html += "<ul>";
                    html += "    <li class=\"home\"><a title=\"Go to Home Page\"  class=\"glyphicon-home\" href=\"/\"> </a>   <i class=\"icon-angle-right\"></i></li>";

                    for (var i = len - 1; i >= 0; i--) {
                        var url = jdata[i].Url; if (url == null) url = "#";
                        html += "    <li class=\"\"><a title=\"" + jdata[i].Title + "\" href=\"" + url + "\">" + jdata[i].Title + "</a> <i class=\"icon-angle-right\"></i></li>";
                        if (i == 0) {
                            if (document.title.length < 1) {
                                document.title = jdata[i].Title;
                            }
                        }
                    }
                    html += "</ul>";
